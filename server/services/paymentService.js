...
export const refundCaptureService = async (captureId, amount) => {
  ...
  if (!response.ok) {
    console.error("❌ Refund PayPal Error Details:", JSON.stringify(data, null, 2)); // הדפסת שגיאה אמיתית
...
export const saveCaptureIdToDb = async (bookingId, captureId) => {
  const db = getDb();
  await db.query("UPDATE bookings SET paypal_capture_id = ? WHERE id = ?", [captureId, bookingId]);
};

export const cancelBookingAndHandleRefund = async (bookingId) => {
  const db = getDb();
  const [rows] = await db.query("SELECT event_date, payment, status, paypal_capture_id FROM bookings WHERE id = ?", [bookingId]);
  const booking = rows[0];
  if (!booking) throw new Error("הזמנה לא נמצאה");
  if (booking.status === "canceled") throw new Error("ההזמנה כבר בוטלה");

  const daysDiff = Math.ceil((new Date(booking.event_date) - new Date()) / (1000 * 60 * 60 * 24));
  ...
  await db.query("UPDATE bookings SET status = 'canceled', cancellation_fee = ? WHERE id = ?", [cancellationFee, bookingId]);
